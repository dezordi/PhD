...
my_comparisons <- list(c("Alpha (B.1.1.7)","B.1.1"),c("Alpha (B.1.1.7)","B.1.1.28"),
                       c("Alpha (B.1.1.7)","B.1.1.33"),c("Alpha (B.1.1.7)", "Delta (B.1.617.2)"),
                       c("Alpha (B.1.1.7)","Delta-sub (AY.x)"),c("Alpha (B.1.1.7)", "Gamma (P.1)"),
                       c("Alpha (B.1.1.7)","Gamma-sub (P.1.x)"),c("Alpha (B.1.1.7)", "Others"),
                       c("Alpha (B.1.1.7)","P.2"),c("B.1.1", "B.1.1.28"),c("B.1.1","B.1.1.33"),
                       c("B.1.1", "Delta (B.1.617.2)"),c("B.1.1","Delta-sub (AY.x)"),
                       c("B.1.1", "Gamma (P.1)"),c("B.1.1","Gamma-sub (P.1.x)"),
                       c("B.1.1", "Others"),c("B.1.1","P.2"),c("B.1.1.28","B.1.1.33"),
                       c("B.1.1.28", "Delta (B.1.617.2)"),c("B.1.1.28","Delta-sub (AY.x)"),
                       c("B.1.1.28", "Gamma (P.1)"),c("B.1.1.28","Gamma-sub (P.1.x)"),
                       c("B.1.1.28", "Others"),c("B.1.1.28","P.2"),c("B.1.1.33", "Delta (B.1.617.2)"),
                       c("B.1.1.33","Delta-sub (AY.x)"),
                       c("B.1.1.33", "Gamma (P.1)"),c("B.1.1.33","Gamma-sub (P.1.x)"),
                       c("B.1.1.33", "Others"),c("B.1.1.33","P.2"),
                       c("Delta (B.1.617.2)","Delta-sub (AY.x)"),
                       c("Delta (B.1.617.2)", "Gamma (P.1)"),c("Delta (B.1.617.2)","Gamma-sub (P.1.x)"),
                       c("Delta (B.1.617.2)", "Others"),c("Delta (B.1.617.2)","P.2"),
                       c("Delta-sub (AY.x)", "Gamma (P.1)"),c("Delta-sub (AY.x)","Gamma-sub (P.1.x)"),
                       c("Delta-sub (AY.x)", "Others"),c("Delta-sub (AY.x)","P.2"),c("Gamma (P.1)","Gamma-sub (P.1.x)"),
                       c("Gamma (P.1)", "Others"),c("Gamma (P.1)","P.2"),
                       c("Gamma-sub (P.1.x)", "Others"),c("Gamma-sub (P.1.x)","P.2"),
                       c("Others","P.2"))

my_comparisons_sig <- list(c("B.1.1","Delta-sub (AY.x)"),
                           c("B.1.1", "Gamma (P.1)"),c("B.1.1","Gamma-sub (P.1.x)"),
                           c("B.1.1", "Others"),c("B.1.1","P.2"),c("B.1.1.28","Delta-sub (AY.x)"),
                           c("B.1.1.28", "Gamma (P.1)"),c("B.1.1.28","Gamma-sub (P.1.x)"),
                           c("B.1.1.33","Delta-sub (AY.x)"),
                           c("B.1.1.33", "Gamma (P.1)"),c("B.1.1.33","Gamma-sub (P.1.x)"),
                           c("B.1.1.33","P.2"),
                           c("Delta-sub (AY.x)", "Gamma (P.1)"),c("Delta-sub (AY.x)","Gamma-sub (P.1.x)"),
                           c("Gamma (P.1)","Gamma-sub (P.1.x)"),
                           c("Gamma (P.1)", "Others"),c("Gamma (P.1)","P.2"),
                           c("Gamma-sub (P.1.x)", "Others"),c("Gamma-sub (P.1.x)","P.2"))
...
ggplot(intrahost_table, aes(x = lineage_up_major, y = iSNVs)) +
  ...
  annotate("text", x='Alpha (B.1.1.7)', y=29, label= paste("Desvio: ",alpha_values$Stdev,sep=' ')) +
  annotate("text", x='B.1.1', y=33, label= paste("Média: ",b11_values$Mean,sep=' '))+
  annotate("text", x='B.1.1', y=29, label= paste("Desvio: ",b11_values$Stdev,sep=' ')) +
  annotate("text", x='B.1.1.28', y=33, label= paste("Média: ",b1128_values$Mean,sep=' '))+
  annotate("text", x='B.1.1.28', y=29, label= paste("Desvio: ",b1128_values$Stdev,sep=' ')) +
  annotate("text", x='B.1.1.33', y=33, label= paste("Média: ",b1133_values$Mean,sep=' '))+
  annotate("text", x='B.1.1.33', y=29, label= paste("Desvio: ",b1133_values$Stdev,sep=' ')) +
  annotate("text", x='Delta (B.1.617.2)', y=33, label= paste("Média: ",delta_values$Mean,sep=' '))+
  annotate("text", x='Delta (B.1.617.2)', y=29, label= paste("Desvio: ",delta_values$Stdev,sep=' ')) +
  annotate("text", x='Delta-sub (AY.x)', y=33, label= paste("Média: ",deltasub_values$Mean,sep=' '))+
  annotate("text", x='Delta-sub (AY.x)', y=29, label= paste("Desvio: ",deltasub_values$Stdev,sep=' ')) +
  annotate("text", x='Gamma (P.1)', y=33, label= paste("Média: ",gamma_values$Mean,sep=' '))+
  annotate("text", x='Gamma (P.1)', y=29, label= paste("Desvio: ",gamma_values$Stdev,sep=' ')) +
  annotate("text", x='Gamma-sub (P.1.x)', y=33, label= paste("Média: ",gammasub_values$Mean,sep=' '))+
  annotate("text", x='Gamma-sub (P.1.x)', y=29, label= paste("Desvio: ",gammasub_values$Stdev,sep=' ')) +
  annotate("text", x='Others', y=33, label= paste("Média: ",outras_values$Mean,sep=' '))+
  annotate("text", x='Others', y=29, label= paste("Desvio: ",outras_values$Stdev,sep=' ')) +
  annotate("text", x='P.2', y=33, label= paste("Média: ",p2_values$Mean,sep=' '))+
  annotate("text", x='P.2', y=29, label= paste("Desvio: ",p2_values$Stdev,sep=' ')) +
...
my_comparisons_group <- list(c("0-15","16-65"),c("0-15","66+"),c("16-65","66+"))
