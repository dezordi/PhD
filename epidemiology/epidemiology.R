...
x_order_month <- c( "2020_01","2020_02", "2020_03", "2020_04",
...
plot_lineage_PE <- ggplot(data_lineages, aes(x=factor(period, levels = x_order_month),y=lineage_freq, fill=lineage)) +
  geom_bar(position = 'fill', stat = 'identity', colour='black',alpha=0.7) +
  geom_line(aes(y=genome_count/600, group=1), colour = 'black', size = 1.5, alpha=0.6)+
  scale_y_continuous(
    name = 'Frequência',
    sec.axis = sec_axis(~.*600, name ='Número de Genomas (n)')
  ) +
  scale_x_discrete(limits = x_order_month) +
  labs(x = 'Período (ano/mês)') +
  theme(legend.position = 'top',
        axis.text.x = element_text(angle = 90)) +
  guides(fill=guide_legend(ncol=12)) +
  scale_fill_manual(values = cols_lineages)
...
plot_cases_PE <- ggplot(cases_genome_by_period, aes(x=factor(period, levels = x_order_month),y=case_number)) +
  geom_bar(stat = 'identity', colour='black',alpha=0.3) +
  geom_line(aes(y=genomesBycases*200000, group=1), colour = 'red', size = 1.5, alpha=0.6)+
  scale_y_continuous(
    name = 'Número de Casos (n)',
    sec.axis = sec_axis(~./200000, name ='Genomas Sequenciados / Número de Casos (%)')
  ) +
  scale_x_discrete(limits = x_order_month) +
  labs(x = '') +
  theme(axis.text.x = element_blank())
...
plot_lineage_RS <- ggplot(data_lineages, aes(x=factor(period, levels = x_order_month),y=lineage_freq, fill=lineage)) +
  geom_bar(position = 'fill', stat = 'identity', colour='black',alpha=0.7) +
  geom_line(aes(y=genome_count/260, group=1), colour = 'black', size = 1.5, alpha=0.6)+
  scale_y_continuous(
    name = 'Frequência',
    sec.axis = sec_axis(~.*260, name ='Número de Genomas (n)')
  ) +
  scale_x_discrete(limits = x_order_month) +
  labs(x = 'Período (ano/mês)') +
  theme(legend.position = 'top',
...
plot_cases_RS <- ggplot(cases_genome_by_period, aes(x=factor(period, levels = x_order_month),y=case_number)) +
  geom_bar(stat = 'identity', colour='black',alpha=0.3) +
  geom_line(aes(y=genomesBycases*30000, group=1), colour = 'red', size = 1.5, alpha=0.6)+
  scale_y_continuous(
    name = 'Número de Casos (n)',
    sec.axis = sec_axis(~./30000, name ='Genomas Sequenciados / Número de Casos (%)')
  ) +
  scale_x_discrete(limits = x_order_month) +
  labs(x = '') +
  theme(axis.text.x = element_blank())
